...
def find_components(edges, max_components=16):
    ...
    while count > 16:
        n += 1
        dilated_image = dilate(edges, N=3, iterations=28)
...
def find_optimal_components_subset(contours, edges, vari):
    ...
    c = c_info[0]
    del c_info[0]
    this_crop = c['x1'], c['y1'], c['x2'], c['y2']-vari
    crop = this_crop
    covered_sum = c['sum']
...
def pad_crop(crop, contours, edges, border_contour, pad_px=15):
    ...
    changed = False
    for c in c_info:
        this_crop = c['x1']-5, c['y1']-5, c['x2']+5, c['y2']-5
...
def process_image(path, out_path, vari):
    ...
    crop = find_optimal_components_subset(contours, edges, vari)
...
def detect_text_region(files, vari):
    for path in files:
        out_path = path.replace('.jpg', '.png')
        out_path = path.replace('full_', '')
        #out_path = path.replace('.png', '.crop.png')  # .png as input
        if os.path.exists(out_path): continue
        try:
            process_image(path, out_path, vari)
